...
# --- Page config ---
st.set_page_config(page_title="DBMS  - Aniket’s Study Saga", layout="wide")
...
">
🔌 {st.session_state.get('subject_name', 'DBMS')}
</h1>
...
""")
# st.markdown("<h4 style='color:#f39c12; font-weight:bold;'>📌 Electricity powers the world. Understand the flow, and you control the current!</h4>", unsafe_allow_html=True)
st.markdown("<h4 style='color:#d35400; font-weight:bold;'>📌 A well-structured database is like a sharp mind — organized, fast, and powerful!</h4>", unsafe_allow_html=True)

# --- PDF Upload ---
uploaded_files = st.file_uploader(
    "Upload your DBMS past paper PDFs (minimum 2):", type=["pdf"], accept_multiple_files=True
)
    ...
    if st.button("Predict Unit-wise Questions"):
        with st.spinner("Analyzing and predicting questions for each unit..."):
            ...
            if unitwise_predictions:
                pdf_file_name = save_unitwise_questions_as_pdf(unitwise_predictions, subject_name="DBMS")
