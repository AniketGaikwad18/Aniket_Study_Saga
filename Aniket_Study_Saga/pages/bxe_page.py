...
# --- Page config ---
st.set_page_config(page_title="BXE - Aniket’s Study Saga", layout="wide")
...
">
🔌 {st.session_state.get('subject_name', 'BXE')}
</h1>
...
""")
st.markdown("<h4 style='color:#2c3e50; font-weight:bold;'>📌 Build circuits. Build logic. Build the future. Your brain is your best microcontroller!</h4>", unsafe_allow_html=True)

# --- PDF Upload ---
uploaded_files = st.file_uploader(
    "Upload your BXE past paper PDFs (minimum 2):", type=["pdf"], accept_multiple_files=True
)
    ...
    if st.button("Predict Unit-wise Questions"):
        with st.spinner("Analyzing and predicting questions for each unit..."):
            ...
            if unitwise_predictions:
                pdf_file_name = save_unitwise_questions_as_pdf(unitwise_predictions, subject_name="BXE")
