...
# --- Page config ---
st.set_page_config(page_title="OS  - Aniket’s Study Saga", layout="wide")
...
">
🔌 {st.session_state.get('subject_name', 'OS')}
</h1>
...
You can download the predicted questions as a PDF named **Predictionbyaniket.pdf**.  

""")
st.markdown("<h4 style='color:#c0392b; font-weight:bold;'>📌 Control the core. The OS is the bridge between your logic and the machine!</h4>", unsafe_allow_html=True)

# --- PDF Upload ---
uploaded_files = st.file_uploader(
    "Upload your OS past paper PDFs (minimum 2):", type=["pdf"], accept_multiple_files=True
)
    ...
    if st.button("Predict Unit-wise Questions"):
        with st.spinner("Analyzing and predicting questions for each unit..."):
            ...
            if unitwise_predictions:
                pdf_file_name = save_unitwise_questions_as_pdf(unitwise_predictions, subject_name="OS")
