...
# --- Page config ---
st.set_page_config(page_title="CG - Aniket’s Study Saga", layout="wide")
...
st.markdown("<h4 style='color:#16a085; font-weight:bold;'>📌 Bring ideas to life with visuals. Graphics is where code meets creativity!</h4>", unsafe_allow_html=True)

# --- PDF Upload ---
uploaded_files = st.file_uploader(
    "Upload your CG past paper PDFs (minimum 2):", type=["pdf"], accept_multiple_files=True
)
...
if uploaded_files and len(uploaded_files) >= 2:
    if st.button("Predict Unit-wise Questions"):
        with st.spinner("Analyzing and predicting questions for each unit..."):
            ...
            st.text_area("Extracted Raw Text", "\n\n".join([f"{k}:\n{v}" for k, v in unit_texts.items()]), height=300)
            # Predict questions for each unit
            ...
            if unitwise_predictions:
                pdf_file_name = save_unitwise_questions_as_pdf(unitwise_predictions, subject_name="Computer Graphics")
