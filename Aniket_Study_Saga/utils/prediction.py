...
def extract_text_by_unit(pdf_files):
    ...
    chunks = re.split(r'(UNIT\s*[-–]?[IVX]+)', full_text, flags=re.IGNORECASE)

    unit_text = defaultdict(str)
    current_unit = None
    for part in chunks:
        part = part.strip()
        if re.match(r'UNIT\s*[-–]?[IVX]+', part, flags=re.IGNORECASE):
...
def save_unitwise_questions_as_pdf(unit_questions_dict, subject_name):
    ...
    file_name = f"Predictionbyaniket_{subject_clean}.pdf"
